...
/// 
/// The `ontologySeries` enables assigning multiple ontologies to each [`VTarget`](crate::misb0903::target::VTarget) (see [`VTarget`](crate::misb0903::target::VTarget) Item 107) and multiple features to each  [`VObject`](crate::misb0903::target::Misb0903Object) (see [`VObject`](crate::misb0903::target::Misb0903Object) Item 5). 
/// 
...
pub struct Misb0903Ontology {
    ...
    /// 
    /// When the VMTI LS contains a Series of Ontology LS, each element in the Series gets a unique identifier. The `ontologyId` item is an integer which identifies a single Ontology LS in the Series and is unique in the list. Systems may reuse `ontologyId`s for different ontologies/entities from VMTI LS to VMTI LS (i.e., two sequential VMTI packets) so receivers should not assume an `ontologyId` value is static for a whole VMTI stream. The `ontologyId` does not need to start at a value of one nor do the `ontologyId`s need to be in any specific order in the Ontology Series.
    /// 
    /// Len: V8
    pub ontology_id: u64,
    ...
    #[klv(key = 0x04)]
    /// (Mandatory) Internationalized Resource Identifier (IRI) specifying an entity from the ontology.
    /// 
    ...
    #[klv(key = 0x05)]
    /// (Optional) Internationalized Resource Identifier (IRI) specifying the version of the ontology. 
    /// 
    ...
    /// 
    /// The `label` is the name of the entity, defined in the entityIRI, as defined by the ontology. See Section 7.3. The `label` text is either the value of the (OWL defined) `rdfs:label` or `skos:prefLabel` property of the entity
