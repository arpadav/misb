use std::f32::consts::E;

use winnow::Parser;

// use misb::test_data::data;

const UHL: &'static [u8] = &[0x06, 0x0E, 0x2B, 0x34, 0x02, 0x0B, 0x01, 0x01, 0x0E, 0x01, 0x03, 0x01, 0x01, 0x00, 0x00, 0x00];

fn uhl_take<'a>(input: &mut &'a [u8]) -> winnow::PResult<&'a [u8]> {
    winnow::token::literal(UHL).parse_next(input)
}

use tinyklv::prelude::*;
use misb::misb0601::Misb0601;
use rand::Rng;

fn main() {
    // let mut rng = rand::thread_rng();
    // let binding = {
    //     let data = data();
    //     let idx = rng.gen_range(0..data.len());
    //     data[idx].clone()
    // };
    // let input = &mut binding.as_slice();

    let mut data: &[u8] = &[0x01, 0x02, 0x04, 0x03];
    let len: usize = 3;
    // let taken: tinyklv::reexport::winnow::PResult<&[u8]> = tinyklv::reexport::winnow::token::take(len).parse_next(&mut data);
    // println!("{:?}", taken);
    let value = tinyklv::binary::dec::be_u16(
        &mut tinyklv::reexport::winnow::token::take::
            <usize, &[u8], tinyklv::reexport::winnow::error::ContextError>(len)
        .parse_next(&mut data)
        .unwrap()
    );
    println!("good value: {:?}", value);
    println!("next data: {:?}", data);

    let mut data: &[u8] = &[0x01, 0x02, 0x04, 0x03];
    let len: usize = 3;
    // let taken: tinyklv::reexport::winnow::PResult<&[u8]> = tinyklv::reexport::winnow::token::take(len).parse_next(&mut data);
    // println!("{:?}", taken);
    let value = simulate_bad_decoder::<u16>(
        &mut tinyklv::reexport::winnow::token::take::
            <usize, &[u8], tinyklv::reexport::winnow::error::ContextError>(len)
        .parse_next(&mut data)
        .unwrap()
    );
    println!("bad value: {:?}", value);
    println!("next data: {:?}", data);

    let mut data: &[u8] = &[
        ,
    ];
    let input = &mut data;

    let uhl_literal: &[u8; 16] = &[0x06, 0x0e, 0x2b, 0x34, 0x02, 0x0b, 0x01, 0x01, 0x0e, 0x01, 0x03, 0x01, 0x01, 0x00, 0x00, 0x00];

    let new_input: winnow::PResult<&[u8]> = (
        winnow::token::take_until(0.., uhl_literal.as_slice()),
        uhl_literal.as_slice()
    )
    .parse_next(input)
    .and_then(|x| {
        println!("debug point");
        println!("{:?}", x);
        // x
    });

    // let res: winnow::PResult<&[u8]> = winnow::token::take_until(0.., uhl_literal.as_slice()).parse_next(input);
    // res.map(|_| {
    //     let _: winnow::PResult<&[u8]> = uhl_literal.as_slice().parse_next(input);
    // });
    // let res2: winnow::PResult<&[u8]> = winnow::token::take_until(0.., uhl_literal.as_slice()).parse_next(input);
    // println!("{:?}", res2);

    // let results: winnow::PResult<Vec<Misb0601>> = winnow::combinator::repeat(0.., Misb0601::extract).parse_next(input);
    // let results2 = Misb0601::extract(input);
    // println!("{:?}", results2);

    // match results {
    //     Ok(results) => for res in results {
    //         println!("{:?}", res);
    //     },
    //     Err(e) => println!("{:?}", e),
    // }

    println!("debug point");
}

fn simulate_bad_decoder<T>(input: &mut &[u8]) -> winnow::PResult<T> {
    // Err(tinyklv::reexport::winnow::error::ErrMode::Incomplete(tinyklv::reexport::winnow::error::Needed::new(1)))
    Err(tinyklv::err!())
}